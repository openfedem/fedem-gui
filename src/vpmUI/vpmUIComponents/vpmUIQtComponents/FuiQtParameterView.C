...
FuiQtParameterView::FuiQtParameterView(QWidget* parent)
  : FFuQtMultUIComponent(parent,"ParameterView")
{
  IAmSensitive = true;

  //TODO Qt4-port: Find out how to use a QScrollArea object here
...
void FuiQtParameterView::setFields(const std::vector<std::string>& names)
{
  ...
  {
    myLayout->addWidget(new QLabel(name.c_str()),row,0);
    FFuQtIOField* field = new FFuQtIOField(this);
