...
long Fd2DPoints::addPoint(const FaVec3& position)
{
  ...
  newnode->id = lastid++;
  newnode->position.setValue(position[0],position[1],position[2]);
  newnode->next = NULL;
  newnode->prev = NULL;
...
void Fd2DPoints::movePoint(long id, const FaVec3& position)
{
  ...
  // Move the point node.
  if(runner) {
    runner->position.setValue(position[0],position[1],position[2]);
#ifdef FD_DEBUG
